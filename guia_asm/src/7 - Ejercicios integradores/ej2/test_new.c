...
#define P(valor_clase, valor_combustible, valor_references)                    \
  (&(attackunit_t){.clase = #valor_clase,                                      \
...
void testResults1A(mapa_t esperado, mapa_t resultado,
                   bool cantidad_refs_correcta, char *testName) {
  bool iguales = cantidad_refs_correcta;
  uint32_t i;
  for (i = 0; iguales && i < 255 * 255; i++) {
    iguales =
        iguales && ((uint64_t *)esperado)[i] == ((uint64_t *)resultado)[i];
  }
...
void test_1a_varios_cambios() {
  ...
  *mapa[4][4] = *P(battleship, 99, 1);
  ...
  testResults1A(esperado, mapa, mapa[1][1]->references == 3,
...
void test_1a_instancia_compartida_nueva() {
  ...
  *compartida = *P(anti - air, 60, 0);
  ...
  testResults1A(esperado, mapa, compartida->references == 3,
...
void test_1a_sin_cambios() {
  ...
  *mapa[4][4] = *P(battleship, 99, 1);
  ...
  testResults1A(esperado, mapa, mapa[0][0]->references == 1,
                "test_1a_sin_cambios");
...
void test_1a_mapa_completo(uint64_t successful_at_start) {
  if (successful_tests - successful_at_start < 4) {
    printf(FAIL "El test sobre el mapa completo se ejecutará cuando pasen los "
                "tests anteriores.\n");
    return;
  }
  mapa_t mapa = {0};
  ...
  *compartida = *P(anti - air, 60, 0);
  ...
  optimizar(mapa, compartida, hash);
  testResults1A(esperado, mapa, compartida->references == 5,
                "test_1a_mapa_completo");
...
void testResults1B(uint32_t esperado, uint32_t resultado, mapa_t mapa,
                   char *testName) {
  ...
  if (iguales) {
    ...
    printf(
        FAIL
        "%s: se contó %d de combustible pero el total correcto es %d. Mapa:\n",
...
void testResults1C(mapa_t esperado, mapa_t resultado,
                   bool cantidad_refs_correcta, char *testName) {
  bool iguales = cantidad_refs_correcta;
  uint32_t i;
  for (i = 0; iguales && i < 255 * 255; i++) {
    iguales = iguales && (hash_test(((attackunit_t **)esperado)[i]) ==
                          hash_test(((attackunit_t **)resultado)[i]));
  }
...
void test_1c_cambio_compartida() {
  ...
  *esperado[1][1] = *P(anti - air, 50, 1);
  ...
  testResults1C(esperado, mapa, mapa[0][0]->references == 1,
...
void test_1c_cambio_individual() {
  ...
  *esperado[3][4] = *P(mech, 50, 1);
  ...
  testResults1C(esperado, mapa, mapa[3][4]->references == 1,
 ...
 *
 * Las variables `EJERCICIO_2A_HECHO`, `EJERCICIO_2B_HECHO` y
 * `EJERCICIO_2C_HECHO` controlan qué testsuites se van a correr. Ponerlas como
 * `false` indica que el ejercicio no está implementado y por lo tanto no querés
 * que se corran los tests asociados a él.
 *
